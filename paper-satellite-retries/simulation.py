...
logging.basicConfig(
    filename="simulation.log", filemode="w", level=logging.INFO, encoding="utf-8"
)
...
class Simulator:
    ...
    @classmethod
    def schedule_event(
        cls, time_at: float, action: Callable[[], None], e_type: EventType
    ) -> int:
    ...
    def cancel_event(cls, event_id: int):
        ...
        if event is not None:
            ...
            logger.warning(
                "%4.5f No Event[id=%s] to cancel", instance.get_current_time(), event_id
            )
    ...
    def run(cls, number_of_events: int):
        ...
        while (
            not instance.event_queue.empty()
            and instance.processed_events < number_of_events
        ):
        ...
        if instance.event_queue.empty():
            logger.warning(
                "%4.5f Simulation ended: empty event queue", instance.get_current_time()
            )
        else:
            logger.info(
...
class Request:
    ...
    def realloc_resources(self, new_alloc_resources: int) -> float:
        ...
        service_time_left = (
            self.total_size - self.served_size
        ) / self.current_alloc_resource
    ...
    def finish_service(self):
        ...
        logger.debug(
            "%4.5f %s: finish service", Simulator.get_current_time(), str(self)
        )
...
class Flow(StatsBaseClass, ABC):
    ...
    def request_serviced(self, req: Request):
        ...
        self.request_service_time = (
            total * self.request_service_time + service_time
        ) / (total + 1)
...
class ElasticDataFlow(Flow):
    ...
    def batch_generated(self, batch_size):
        ...
        self.requests_per_batch = (total * self.requests_per_batch + batch_size) / (
            total + 1
        )
...
class Network(StatsBaseClass):
    ...
    def update_utilization_stats(self):
        ...
        for flow_id in self.flows_lookup:
            self.mean_flow_reqs_in_service[flow_id] += (
                duration * flow_reqs_in_service[flow_id]
            )
            self.mean_flow_res_in_service[flow_id] += (
                duration * flow_res_in_service[flow_id]
            )
    ...
    def add_flows(self, n_real_time_flows: int, n_elastic_flows: int):
        ...
        logger.info(
            "Created flows: RT = %s, Data = %s", n_real_time_flows, n_elastic_flows
        )
    ...
    def redistribute_elastic_resources(self):
        ...
        real_time_resources = sum(
            req.current_alloc_resource for req in real_time_requests
        )
        ...
        total_distributed = (
            rfloor_res * num_req_floor_res + (rfloor_res + 1) * num_req_ceil_res
        )
...
def convert_stats_to_metrics(params: ParametersSet, network: Network):
    ...
    flows_stats = {
        flow_id: network.flows_lookup[flow_id].get_stats()
        for flow_id in network.flows_lookup
    }

    has_ed = any(
        isinstance(flow, ElasticDataFlow) for flow in network.flows_lookup.values()
    )
    has_rt = any(
        isinstance(flow, RealTimeFlow) for flow in network.flows_lookup.values()
    )
    ...
    if has_rt:
        for i in range(n_rt_flows):
            ...
            metrics.mean_rt_requests_in_service[i] = net_stats[
                "mean_flow_reqs_in_service"
            ][i]

            metrics.mean_resources_per_rt_flow[i] = net_stats[
                "mean_flow_res_in_service"
            ][i]
    ...
    if has_ed:
        ...
        metrics.mean_data_requests_in_service = net_stats["mean_flow_reqs_in_service"][
            key
        ]
        metrics.mean_resources_per_data_flow = net_stats["mean_flow_res_in_service"][
            key
        ]
...
def main():
    ...
    network.add_flows(
        n_real_time_flows=params.real_time_flows,
